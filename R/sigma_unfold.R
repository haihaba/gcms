sigma_unfold  <-  function(projectpath)
{
	if(!file.exists(file.path(projectpath,"Aligned","shift.Rdata")))
	{
	...
	}else
	{
		...
		if(file.exists(file.path(projectpath,"Edges","edges.Rdata")))
		{
			...
			load(file.path(projectpath,"Aligned","NUM_scans.Rdata"))
			...
			dir.create(file.path(projectpath,"H_Dcomp"),showWarnings=FALSE)
			save(DATA,OBSID1,file=file.path(projectpath,"H_Dcomp","TOT_FILE.Rdata"))
			save(edges,shift,ROWID1,files,file=file.path(projectpath,"info.Rdata"))
			output  <-  DATA
		}else
		{
